...
<style>
  ...
  a.btn-secondary, button.btn-danger {
    margin-top: 30px;
    margin-left: 15px;
    padding: 10px 24px;
    border-radius: 30px;
...
<div class="container">
  ...
  <form action="${ctx}/admin/bookManage/save" method="post">
    ...
    <a href="${ctx}/admin/bookManage" class="btn btn-secondary">목록으로 돌아가기</a>
  </form>

  <c:if test="${book.bookId != null}">
    <form action="${ctx}/admin/bookManage/delete/${book.bookId}" method="post" style="display:inline;">
      <!-- CSRF 토큰 추가 -->
      <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
